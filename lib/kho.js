...
/** Chuỗi ngày ghi liên tiếp tính tới hôm nay (hoặc hôm qua nếu hôm nay chưa ghi). */
export function chuoiNgay() {
  let n = 0;
  const d = new Date();
  if (!docNgay(khoaNgay(d)).length) d.setDate(d.getDate() - 1);
  for (let i = 0; i < 400; i++) {
    if (!docNgay(khoaNgay(d)).length) break;
    n++;
...
/** Xoá nhật ký cũ hơn N ngày để localStorage không phình vì ảnh thu nhỏ. */
export function donKho(giuNgay = 60) {
  if (typeof window === "undefined") return;
  const moc = new Date();
  moc.setDate(moc.getDate() - giuNgay);
  const mocKhoa = khoaNgay(moc);
  try {
    for (const k of Object.keys(localStorage)) {
      if (k.startsWith("mam:nhatky:") && k.slice(11) < mocKhoa) localStorage.removeItem(k);
    }
  } catch {}
}
